...
// Proxy is a service to proxy push notification
type Proxy struct {
...
// NewProxyService new instance of proxy service
func NewProxyService(n *NotificationClient, c cryptoService, cs cachingService, host string) *Proxy {
	return &Proxy{
		notification:   n,
		cryptoService:  c,
		cachingService: cs,
...
func (ps *Proxy) SendNotification(ctx context.Context, msg *Message) ([]NotificationResult, error) {
	...
	for _, m := range msg.PushMetadata.Devices {
		...
		device, err := decryptDeviceInfo(m, ps.cryptoService)
		if err != nil {
	...
	err := ps.cachingService.Set(ctx, id, msgToSend, time.Hour*24)
	if err != nil {
		log.Error(err)
	...
	contentBody := struct {
		...
		ID:  id,
		URL: fmt.Sprintf("%s/api/v1/%s", ps.hostURL, id),
	}
	...
	rejectedTokens, err := ps.notification.Notify(ctx, devices, c)
	if err != nil {
		log.Error(err)
		return nil, errors.New("failed to notify devices")
	}
	...
	for token, enc := range decryptedMap {
		...
		if isRejected {
			msgProcessingResult = append(msgProcessingResult, NotificationResult{
				...
				Reason: "Push message could have been rejected by an upstream gateway because they have expired or have never been valid",
			})
			continue
		}
		msgProcessingResult = append(msgProcessingResult, NotificationResult{
...
func decryptDeviceInfo(enc EncryptedDeviceMetadata, cr cryptoService) (Device, error) {
